...
- **通俗讲解版本**：如果你觉得本文内容较为抽象，可以阅读 [《第一节-Solana 核心概念通俗讲解》]({{ site.baseurl }}/2026/01/07/solana-lesson1-core-concepts-explained.html)，该文章用更生活化的语言和类比来解释 Solana 的核心概念
...
使用 loader-v3 部署的程序，其 data 字段中不包含程序代码。相反，其 data 指向一个单独的 **程序数据账户**（Program Executable Data Account），该账户包含程序代码。

**关键特征**：
- **存储内容**：程序自身的代码数据（可执行的二进制代码）
- **Owner**：BPF Loader
- **创建时机**：程序部署时由 BPF Loader 自动创建
- **用途**：存储程序的源代码编译后的可执行代码
- **可执行**：False（虽然存储代码，但账户本身不可执行）

**重要理解**：程序数据账户存储的是程序自身的数据（代码），而不是程序运行时的业务数据。
...
**关键特征**：
- **存储内容**：程序运行时产生的业务数据（用户数据、状态信息等）
- **Owner**：程序本身（program_id）
- **创建时机**：程序运行时通过 CPI 调用 System Program 创建
- **用途**：存储程序的业务状态，如用户余额、NFT 元数据、游戏分数等
- **可执行**：False（纯数据账户）

**重要理解**：程序状态账户存储的是程序运行时产生的业务数据，而不是程序自身的代码数据。每个用户或每个业务实体通常有自己独立的状态账户。
...
为了更清晰地理解这两个概念的区别，我们用一个对比表来说明：

| 特征 | 程序数据账户（Program Executable Data Account） | 程序状态账户（Program State Data Account） |
|------|-----------------------------------------------|------------------------------------------|
| **存储内容** | 程序自身的代码数据（可执行二进制代码） | 程序运行时产生的业务数据（用户数据、状态信息） |
| **Owner** | BPF Loader | 程序本身（program_id） |
| **创建时机** | 程序部署时（一次性） | 程序运行时（可多次创建） |
| **创建者** | BPF Loader | 程序通过 CPI 调用 System Program |
| **数量** | 每个程序只有 1 个 | 每个程序可以有多个（每个用户/实体一个） |
| **可执行** | False | False |
| **典型例子** | 智能合约的编译后代码 | 用户余额、NFT 元数据、游戏分数、订单信息 |

**记忆技巧**：
- **程序数据账户** = 程序的"安装包"（存储程序代码）
- **程序状态账户** = 程序的"数据库"（存储业务数据）
...
**第 1 题**：Solana 网络上所有数据存储在哪里？账户地址本质上是什么？

**第 2 题**：账户结构体有哪 5 个字段？各自的作用是什么？

**第 3 题**：`owner` 字段的含义是什么？为什么说它是 Solana 的核心安全机制？

**第 4 题**：程序数据账户和程序状态账户有什么区别？（从存储内容、owner、创建时机三个维度回答）

**第 5 题**：系统账户（System Account）是什么？为什么钱包账户是系统账户？

**第 6 题**：一个 Instruction（指令）包含哪三个字段？各字段的作用是什么？

**第 7 题**：Transaction 和 Instruction 是什么关系？Transaction 有哪两个顶层字段？

**第 8 题**：交易的 `recent_blockhash` 有什么作用？它会在多少个区块后过期？

**第 9 题**：交易总大小上限是多少字节？`account_keys` 数组中账户的排列顺序是什么？

**第 10 题**：基础交易费用如何计算？这笔费用如何分配？

**第 11 题**：优先费用（Priority Fee）怎么计算？为什么要按**请求的** CU 计算而不是**实际使用的** CU？

**第 12 题**：什么是 PDA（程序派生地址）？它和普通公钥地址最核心的区别是什么？

**第 13 题**：派生 PDA 时，`bump seed` 是什么？从哪个值开始迭代？

**第 14 题**：什么是 CPI（跨程序调用）？CPI 的最大调用深度是多少？
...
Solana 网络上所有数据都存储在**账户**中。Solana 网络可以视为一个包含单一账户表的公共数据库，账户与地址之间的关系类似键值对。

账户地址是一个 **32 字节的唯一 ID**，通常以 base58 编码字符串显示，大多数账户使用 Ed25519 公钥作为其地址。
...
| 字段 | 作用 |
|---|---|
| **lamports** | 账户中持有的 lamports 数量（SOL 余额） |
| **data** | 账户存储的数据（程序代码或业务状态） |
| **owner** | 拥有该账户的程序 ID，只有 owner 程序能修改 data |
| **executable** | 指示账户是否包含可执行二进制文件 |
| **rent_epoch** | 已弃用的租金 epoch 字段 |
...
`owner` 字段表示**哪个程序有权限修改这个账户的 `data` 字段**。只有 owner 程序才能写入账户数据。

这是核心安全机制的原因：
- `Program Account` 的 owner 是 `BPF Loader`：只有 BPF Loader 能升级程序代码
- `Data Account` 的 owner 是你的程序：只有你的程序能按业务规则修改状态数据
- 钱包的 owner 是 `System Program`：只有 System Program 能修改 SOL 余额，防止任意程序盗取资金
...
| 维度 | 程序数据账户 | 程序状态账户 |
|---|---|---|
| **存储内容** | 程序自身的代码（可执行二进制） | 程序运行时产生的业务数据 |
| **owner** | BPF Loader | 程序本身（program_id） |
| **创建时机** | 程序部署时（一次性） | 程序运行时（可多次创建） |

记忆技巧：程序数据账户 = 程序的"安装包"；程序状态账户 = 程序的"数据库"。

---

**第 5 题答案**

系统账户是指 **owner 字段为 System Program** 的账户，最典型的例子就是用户的钱包账户。

钱包是系统账户的原因：
- 只有 System Program 能修改钱包的 `lamports` 字段，保证转账安全
- 系统账户可以**支付交易费**（只有 System Program 能扣除 lamports）
- 用户之间转 SOL 本质上是修改两个系统账户的 lamports 字段
...
| 字段 | 作用 |
|---|---|
| **program_id** | 被调用程序的地址，决定"谁来执行" |
| **accounts** | 涉及账户的元数据数组，决定"操作哪些账户" |
| **data** | 字节数组，指定调用哪条指令及所需参数，决定"具体做什么" |

三个字段缺一不可：没有 `program_id` 不知道调用谁，没有 `accounts` 不知道操作什么，没有 `data` 不知道做什么。

---

**第 7 题答案**

Transaction 是**装载 Instruction 的容器**，一个 Transaction 可以包含多个 Instruction，按数组顺序依次执行，且具有**原子性**（一条失败则全部回滚）。

Transaction 的两个顶层字段：
- **signatures**：签名数组，每个需要签名的账户提供一个
- **message**：交易信息，包含 header、account_keys、recent_blockhash、instructions

---

**第 8 题答案**

`recent_blockhash` 是一个哈希值，作用：
1. **作为交易的时间戳**：标识交易创建的时间点
2. **防止重复交易**：相同内容的交易因 blockhash 不同而被视为不同交易

区块哈希在 **150 个区块**后过期（约 1 分钟，假设每个区块 400 毫秒），过期后交易无法被处理。

---

**第 9 题答案**

**交易总大小上限**：**1232 字节**（包含 signatures 和 message）。

**account_keys 排列顺序**（必须严格遵守）：
1. 签名者 + 可写
2. 签名者 + 只读
3. 非签名者 + 可写
4. 非签名者 + 只读

严格排序配合 header 信息，让 Solana 不需要单独存储每个账户的权限，节省空间。

---

**第 10 题答案**

**计算方式**：每个签名收取 **5000 lamports**，有几个签名就乘以几。

**分配方式**：
- **50% 销毁**：从 SOL 流通供应中永久移除
- **50% 给验证者**：支付给处理该交易的验证者

注意：只有 **System Program 拥有的账户**（即系统账户/钱包）才能支付交易费。

---

**第 11 题答案**

**计算公式**：`优先费用 = CU limit × CU price`

**为什么按请求的 CU 而非实际使用的 CU 计算**：
优先费用的本质是"占位费"——你向网络申请了多少计算资源的名额，就按名额收费，不管最终用没用完。

因此：**CU limit 设置过高会多付费**，建议先通过模拟交易估算实际用量，再加 10% 安全余量来设置 CU limit。

---

**第 12 题答案**

PDA（程序派生地址）是使用**程序 ID + 可选种子**确定性生成的地址，与普通公钥最核心的区别：

| | 普通公钥地址 | PDA |
|---|---|---|
| **私钥** | 存在对应私钥 | 没有对应私钥 |
| **签名方式** | 持有私钥的用户签名 | 由程序直接签名（无需私钥） |
| **位置** | 在 Ed25519 曲线上 | 被有意派生在 Ed25519 曲线之外 |

PDA 使程序能够以确定性方式管理账户，无需跟踪账户地址。

---

**第 13 题答案**

`bump seed` 是附加到可选种子上的一个额外字节，用于确保生成的地址落在 Ed25519 曲线之外（即有效的 PDA）。

迭代方向：从 **255** 开始，每次递减 1（255 → 254 → 253 → ...），直到找到第一个有效的非曲线地址为止。找到的第一个有效值称为 **canonical bump**。

---

**第 14 题答案**

CPI（跨程序调用）是指**一个 Solana 程序直接调用另一个程序的指令**。类比为 API 端点在内部调用另一个端点，使程序具有可组合性。

**最大调用深度**：**4 层 CPI**
- 堆栈高度上限为 `MAX_INSTRUCTION_STACK_DEPTH = 5`
- 初始交易本身占 1 层，因此 CPI 最多嵌套 **4 层**

进行 CPI 时，调用程序的账户权限（签名权限、可写权限）会传递给被调用程序。
