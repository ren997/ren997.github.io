...
在讲解更新策略之前，我们先深入理解 `entry` 方法，这是 HashMap 最强大的 API 之一。

#### 什么是 Entry？

`entry` 方法返回一个 `Entry` 枚举，代表 HashMap 中某个键的"入口"：

```rust
pub enum Entry<'a, K, V> {
    Occupied(OccupiedEntry<'a, K, V>),  // 键存在
...
1. **`Occupied`（已占用）**：键已经存在，包含对现有值的可变引用
2. **`Vacant`（空缺）**：键不存在，可以插入新值
...
// ❌ 传统方式：需要两次查找
if !map.contains_key("key") {  // 第一次查找
    map.insert("key", 0);       // 第二次查找
}

// ✅ Entry API：只需一次查找
map.entry("key").or_insert(0);  // 一次查找
```
...
##### 1. `or_insert(default)` - 插入默认值

如果键不存在，插入默认值；返回值的可变引用。
...
// 键不存在，插入 0
let value = map.entry("count").or_insert(0);
*value += 1;
println!("{:?}", map);  // {"count": 1}
...
##### 2. `or_insert_with(f)` - 惰性插入

如果键不存在，调用闭包生成值；返回值的可变引用。
...
// 只有在键不存在时才调用闭包
let value = map.entry("expensive").or_insert_with(|| {
    println!("计算昂贵的值...");
...
**为什么使用 `or_insert_with`？**

```rust
// ❌ or_insert：总是计算默认值（即使不需要）
map.entry("key").or_insert(expensive_computation());

// ✅ or_insert_with：只在需要时才计算
...
##### 3. `or_default()` - 插入类型的默认值

如果键不存在，插入类型的默认值（需要实现 `Default` trait）。
...
// Vec<i32> 的默认值是空 Vec
map.entry("numbers").or_default().push(1);
map.entry("numbers").or_default().push(2);
...
##### 4. `and_modify(f)` - 修改现有值

如果键存在，调用闭包修改值；然后可以链式调用其他方法。
...
// 键不存在的情况
map.entry("new")
   .and_modify(|v| *v += 1)  // 不会执行
   .or_insert(0);             // 执行这个

println!("{:?}", map);  // {"count": 2, "new": 0}
```
...
for (key, value) in data {
    // 使用 or_default 获取或创建 Vec
    grouped.entry(key).or_default().push(value);
}

println!("{:?}", grouped);
// {"apple": [1, 3], "banana": [2, 5], "cherry": [4]}
```
...
fn fibonacci(n: u32, cache: &mut HashMap<u32, u64>) -> u64 {
    // 使用 or_insert_with 避免重复计算
...
fn main() {
    ...
    println!("fib(10) = {}", fibonacci(10, &mut cache));
    println!("缓存: {:?}", cache);
}
```
...
stats.entry("scores")
     ...
     .or_insert(Stats { count: 1, sum: 100 });

println!("{:?}", stats);
// {"scores": Stats { count: 1, sum: 100 }}
...
stats.entry("scores")
     ...
     .or_insert(Stats { count: 1, sum: 200 });

println!("{:?}", stats);
// {"scores": Stats { count: 2, sum: 300 }}
```

#### Entry API 方法对比

| 方法 | 键不存在时 | 键存在时 | 返回值 | 使用场景 |
|------|----------|---------|--------|---------|
| `or_insert(v)` | 插入 `v` | 不变 | `&mut V` | 简单默认值 |
| `or_insert_with(f)` | 插入 `f()` | 不变 | `&mut V` | 昂贵的默认值 |
| `or_default()` | 插入 `V::default()` | 不变 | `&mut V` | 类型有默认值 |
| `and_modify(f)` | 不变 | 调用 `f(&mut v)` | `Entry` | 链式修改 |
| `key()` | - | - | `&K` | 获取键引用 |
...
```
传统方式（两次查找）：
1. contains_key("key") → 计算哈希 → 查找表 → 返回 bool
2. insert("key", value) → 计算哈希 → 查找表 → 插入

Entry API（一次查找）：
1. entry("key") → 计算哈希 → 查找表 → 返回 Entry
2. or_insert(value) → 直接使用 Entry（不需要再次查找）
```
